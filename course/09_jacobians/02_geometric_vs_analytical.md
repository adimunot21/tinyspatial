...
and they are not the same. Knowing which one you're talking about avoids a
...
axes. It composes cleanly with the adjoint when you change frames. It's what
...
(angles you'd plot), not abstract twist components. But:
...
- $J_a$ has **singularities of its own**, separate from the physical
  ...
  $\theta = \pm\pi/2$. Your IK fails not because the arm is stuck, but
...
For visualising things along a chosen parameterisation, or for plugging into
a controller that explicitly accepts RPY-velocity feedback. In both cases
you can convert from $J_g$ to $J_a$ by multiplying by a rotation
parameterisation Jacobian on the left. We don't ship that conversion;
when the IK chapter gets here we'll explain why you almost certainly don't
need to.
...
"geometric vs analytical" is the parameterisation distinction. Confusing!
The course always says "twist Jacobian" or "RPY Jacobian" when we want to
be unambiguous.
