...
TEST_F(CrossTest, ForceIsNegativeTransposeOfMotion) {
  for (int i = 0; i < 20; ++i) {
    const Vector6 m = RandomMotion();
    ExpectNear(cross_force(m), Matrix6(-cross_motion(m).transpose()), 1e-15);
  }
}
...
TEST_F(CrossTest, JacobiIdentity) {
  ...
  for (int i = 0; i < 20; ++i) {
    const Vector6 a = RandomMotion();
    const Vector6 b = RandomMotion();
...
TEST_F(CrossTest, CrossMotionIsAdjointDerivative) {
  ...
  for (int i = 0; i < 20; ++i) {
    ...
    ExpectNear(fd, cross_motion(m), 1e-6);
  }
}
